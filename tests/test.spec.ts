...
test('product page', async ({ page }) => {
    ...
    await page.waitForURL('http://localhost:3000/products?collection=Kai+Shi+%28%E5%BC%80%E5%A7%8B%29');
...
test('purchase process', async ({ page }) => {
    ...
    await page.waitForURL('http://localhost:3000/return?session_id=cs_test_**', { timeout: 0 });
...
test('login admin', async ({ page }) => {
    ...
    await page.waitForURL('http://localhost:3000/dashboard/orders');
    ...
    await page.waitForURL('http://localhost:3000/dashboard/orders/1');
